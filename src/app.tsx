// app.tsx
import { JSX, onMount, Suspense, type Component, createSignal, createEffect, onCleanup } from 'solid-js';
...
const App: Component<{ children: JSX.Element }> = (props) => {
  ...
  createEffect(() => {
    ...
    // Language
    const lang = store.watchSession("lang", "es")()
    if (lang) {
      i18next.changeLanguage(lang)
    }

    // Theme
    const theme = store.watchSession('theme')(); // Esto ES un signal

    if (theme === 'default' || theme === undefined) {
      // Escuchar cambios en preferencia del sistema (reactivo)
      const mediaQuery = window.matchMedia('(prefers-color-scheme: dark)');

      const updateFromSystem = (e: MediaQueryListEvent | MediaQueryList) => {
        document.documentElement.classList.toggle('dark', e.matches);
      };

      // Aplicar valor inicial
      updateFromSystem(mediaQuery);

      // Escuchar cambios futuros
      mediaQuery.addEventListener('change', updateFromSystem);

      // Cleanup
      onCleanup(() => mediaQuery.removeEventListener('change', updateFromSystem));

    } else {
      // Usar el valor del store (reactivo)
