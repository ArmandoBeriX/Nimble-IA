...
export function FormatAttachmentEdit(props: FieldProps) {
  if (!props.field.isEditable) return <></>;
  ...
  const setStaggedFilesAndCache = (files: Array<File | { url: string; name: string }>) => {
    ...
    store.setSession('val_' + (props.record?.id ?? props.field.tableIdentifier ?? createSimpleHash(props.record)) + '_' + props.field.identifier, files)
  };
