...
export default function FormatDateEdit(props: FieldProps) {
  ...
  if (isFieldForm && !props.field.isEditable) {
    return <></>;
  }
  ...
  const getInitialRaw = createMemo(() => {
    const sessionKey = 'val_' + (props.record?.id ?? props.field.tableIdentifier ?? createSimpleHash(props.record)) + '_' + props.field.identifier;
    ...
    return (
      <div class="form-group field-date">
        <WithTooltip tooltip={props.field.description}>
          <label for={fid} class="field-label">
            {props.field.name}
