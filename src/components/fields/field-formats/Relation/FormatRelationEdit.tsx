...
const FormatRelationEdit = (props: FieldProps) => {
  ...
  const relTable = store.getTable(field.relationTableIdentifier!)!;
  ...
  const normalizeToIds = (value: any): any => {
    ...
    if (field.multiple) {
      ...
      return value.map(v => v?.[storageKey] ?? v);
    }
    return value?.[storageKey] ?? value;
  };
  ...
  const idsToObjects = async (keys: any): Promise<any> => {
    ...
    const objects = await store.query(field.relationTableIdentifier!, { [storageKey]: normalizedKeys });
  ...
  const toOption = (rec: any, idx = 0) => {
    ...
    if (!rec) return null;
    if (rec.value && rec.label && rec.id !== undefined) return rec;

    const id = rec.id || `idx-${idx}`;
  ...
  const makeInitialSelected = async () => {
    const rawValue = await getInitialValue();
  ...
  const buildSearchQuery = (input: string) => {
    if (!input) return field.relationQuery || {};
    ...
    if (searchableFields.length === 0) return field.relationQuery || {};
  ...
  const filterable = (inputValue: string) => {
    ...
    try {
      // Filtrado local sobre opciones ya cargadas
      return fuzzySort(inputValue, options(), "text").map((result) => ({
        ...result.item,
        label: result.item.text,
      }));
    } catch (e) {
  ...
  onMount(async () => {
    ...
    ctrl.reset = () => {
      const sessionKey = `val_${props.record?.id ?? props.field.tableIdentifier}_${props.field.identifier}`;
      store.deleteSession(sessionKey);
      makeInitialSelected().then((initial: any) => {
