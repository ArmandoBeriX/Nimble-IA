...
const getPath = (obj: any, path: string) => {
  ...
  for (const alt_path of alternatives) {
    ...
    let cur = obj; // current sub-object
    for (const p of parts) {
      if (typeof cur !== 'object') {
        if (!cur) console.warn(`FormatInterpreter: '${cur}' is not an objet to call '${p}'. Format: ${path}`)
        cur = undefined
        break
      }
      cur = cur[p];
      if (typeof cur === 'object' && !cur.hasOwnProperty(p) && p.endsWith('_id'))
        cur = cur[p.slice(0, -3)];
    }
    if (cur) return cur;
  }
...
export function renderLabelToString(template: string | undefined, rec: any): string {
  if (!template) return "";
...
export function recordDysplay(table: Partial<TableDef>, record: any, format?: string): JSXElement {
  const icon = table?.icon ? <Icon name={table?.icon} /> : <></>;
