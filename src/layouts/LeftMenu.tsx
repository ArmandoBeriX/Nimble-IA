...
export default function LeftMenu() {
  ...
  const isAdmin = createMemo(() => Boolean(user()?.admin));
  ...
  const menuHierarchy = createMemo(() => {
    const items = (allMenuItems || []).filter(item => { // TODO: Patche de Permisos
      const route = item.interface?.route ?? item.identifier ?? '';
      return isAdmin() || !route.startsWith('/admin');
    });
    const mapIds = new Map((searchedMenuItems || []).map(m => [m.id, true])); //fix
  ...
  createEffect(() => {
    const items = (allMenuItems || []).filter(item => { // TODO: Parche de permisos
      const route = item.interface?.route ?? item.identifier ?? '';
      return isAdmin() || !route.startsWith('/admin');
    });
      ...
      <div class={`w-full ${hasChildren && isExpanded() ? 'border-b' : ''} ${item.hidden && !anyChildIsVisible(item.children) ? 'hidden' : ''}`}>
        <div class={`flex items-center justify-between rounded-lg transition-colors`} style={{ 'padding-left': `${Math.min(level * 8, 80)}px` }}>
          {hasChildren ? (
            ...
            <WithTooltip tooltip={item.description} placement="right" contentClass='w-full'>
  ...
  const menuClasses = createMemo(() => {
    const baseClasses = "h-[calc(100vh-48px)] bg-white border-r border-gray-200 overflow-y-auto overflow-x-hidden transition-all duration-300 ease-in-out";
    if (isMenuOpen()) {
