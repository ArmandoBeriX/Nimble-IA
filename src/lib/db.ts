...
import { SyncStatusLabels, TableDef, TableField } from "../types/schema";
...
const metaFields: Omit<TableField, 'id'>[] = [
  {
    identifier: "id",
    name: "ID",
    description: "Identificador único, clave primaria",
    fieldFormat: "string",
    position: 0,
  },
  {
  ...
  },
  {
    ...
    name: "Sincronización",
    description: "Estado de sincronización del registro con el servidor",
    fieldFormat: "list",
    storeData: {
      posibleValues: Object.fromEntries(
        Object.entries(SyncStatusLabels).map(([key, label], index) => [
          key,
  ...
  public async migrateSchema(tableSpec: TableDef[]): Promise<{ migrated: boolean; newVersion?: number }> {
    ...
    for (const t of tableSpec) {
      for (const field of metaFields) {
        ...
        const exists = augmentedFields.find(f => f.tableIdentifier === t.identifier && f.identifier === field.identifier);
        if (exists) continue;

        // Generar id consistente pero único
        const genId = `__meta_${t.identifier}_${field.identifier}`;
        const tf = {
          id: genId,
