...
const SUBSCRIBE_MAX_WAIT = 500
const SUBSCRIBE_DEBOUNCE_DELAY = 100
...
export class RecordsManager {
  ...
  public dbManager: NimbleDBManager;
  ...
  constructor(dbManager: NimbleDBManager) {
    this.db = dbManager.dbInstance();
  ...
  async insert(table: string, records: any): Promise<string[]> {
    ...
    for (const r of arr) {
      if (!r.id) r.id = (typeof crypto !== "undefined" && (crypto as any).randomUUID)
  ...
  async update(table: string, records: any, key: string = 'id'): Promise<void> {
    ...
    if (key === 'id') {
      ...
      if (!keys.length) throw new Error(`update: no se encontró la clave '${key}' en los records`);
  ...
  async delete(table: string, ids: string | number | Array<string | number>, force = false) {
    ...
    if (force) {
      ...
      if (forceIds.length > 0) await t.where("id").anyOf(forceIds).delete();
    ...
    this.logger.debug(`Deleted ${idArr.length} records from ${table}`);
  }

  async query<T = TableRecord>(table: string, rawFilters?: FilterInput, options?: { order?: Array<[string, 'ASC' | 'DESC']>; page?: number; limit?: number }): Promise<T[]> {
    const t = this.db.table(table);
    this.ensureTableInvalidationWatch(table);
  ...
  subscribeToQuery(
    ...
    options: any,
    callback: (data: TableRecord[]) => void,
    includes?: IncludeRelProps,
    onLoading?: () => void,
  ): () => void {
    const manager = this;
    const mainTable = (this.db as any).table(tableId);
    ...
    let canceled = false;
    const ensureActive = () => !canceled;

    let currentRows: TableRecord[] = [];
    let currentIds = new Set<string>();

    // ── resolveIncludes (función pura) ───────────────────────────────────
    const resolveIncludes = async (
      data: TableRecord[],
      includesMap?: IncludeRelProps,
    ): Promise<TableRecord[]> => {
      if (!includesMap || Object.keys(includesMap).length === 0 || data.length === 0) {
        return data;
      }

      const fetched = await Promise.all(
        Object.entries(includesMap).map(async ([key, relProps]) => {
          const fk = relProps.foreignKey || key;
          const tableSource = relProps.tableSource || tableId;
          const field = manager.getField(fk, tableSource);
          const relFieldName = parseRelationField(key);

          if (!field) {
            manager.logger.warn(`subscribeToQuery: field '${fk}' not found on '${tableSource}'`);
            return { relFieldName, map: new Map(), lookupKey: fk };
          }

          const relationType: 'direct' | 'reverse' = tableSource === tableId ? 'direct' : 'reverse';
          const relationKey = field.relationKey || 'id';
          const filterKey = relationType === 'direct' ? relationKey : fk;
          const relTableId = relationType === 'direct' ? field.relationTableIdentifier! : tableSource;
          const lookupKey = relationType === 'direct' ? fk : relationKey;

          const fkValues = Array.from(
            new Set(data.map(d => d[lookupKey]).filter(v => v != null))
          );

          let subdata: TableRecord[] = [];
          if (fkValues.length > 0) {
            try {
              const subFilter = {
                ...(unwrapSignal(relProps.filters) || {}),
                [filterKey]: fkValues,
              };
              subdata = await manager.query(relTableId, subFilter, unwrapSignal(relProps.options));
            } catch (e) {
              manager.logger.warn(`Error loading include '${key}' for ${subscriptionKey}:`, e);
            }
          }

          const enrichedSubdata = relProps.includes && subdata.length > 0
            ? await resolveIncludes(subdata, relProps.includes)
            : subdata;

          const map: Map<string, TableRecord | TableRecord[]> = relationType === 'direct'
            ? new Map(enrichedSubdata.map(s => [String(s[filterKey]), s]))
            : enrichedSubdata.reduce((m, s) => {
              const k = String(s[filterKey]);
              if (!m.has(k)) m.set(k, []);
              (m.get(k) as TableRecord[]).push(s);
              return m;
            }, new Map<string, TableRecord[]>());

          return { relFieldName, map, lookupKey };
        })
      );

      return data.map(item => {
        const extra: Record<string, any> = {};
        for (const { relFieldName, map, lookupKey } of fetched) {
          const val = item[lookupKey];
          extra[relFieldName] = val != null ? (map.get(String(val)) ?? null) : null;
        }
        return { ...item, ...extra };
      });
    };

    // ── collectIncludeTables ─────────────────────────────────────────────
    const collectIncludeTables = (inc?: IncludeRelProps, fromTable = tableId): Set<string> => {
      const tables = new Set<string>();
      if (!inc) return tables;
      for (const [key, relProps] of Object.entries(inc)) {
        const fk = relProps.foreignKey || key;
        const tableSource = relProps.tableSource || fromTable;
        const field = manager.getField(fk, tableSource);
        if (!field) continue;
        const relationType = tableSource === fromTable ? 'direct' : 'reverse';
        const relTableId = relationType === 'direct' ? field.relationTableIdentifier! : tableSource;
        if (relTableId) {
          tables.add(relTableId);
          collectIncludeTables(relProps.includes, relTableId).forEach(t => tables.add(t));
        }
      }
      return tables;
    };

    const includeTables = collectIncludeTables(includes);
    const hasIncludes = !!includes && Object.keys(includes).length > 0;

    manager.logger.debug(`${subscriptionKey}: include tables = [${[...includeTables].join(', ')}]`);

    // ── runFullRefresh ───────────────────────────────────────────────────
    // ✅ Sin shouldRefreshQuery: si liveQuery dispara, siempre refrescamos.
    //    El debounce gestiona la frecuencia. shouldRefreshQuery tenía demasiados
    //    falsos negativos para operaciones UPDATE.
    let isFullRunning = false;
    const runFullRefresh = async () => {
      if (!ensureActive() || isFullRunning) return;
      isFullRunning = true;
      try {
        try { onLoading?.(); } catch { }

        const rows = await manager.query(tableId, filters, options);
        if (!ensureActive()) return;

        currentRows = rows;
        currentIds = new Set(rows.map(r => String(r.id)));

        if (hasIncludes) {
          const enriched = await resolveIncludes(rows, includes);
          if (ensureActive()) callback(enriched);
        } else {
          callback(rows);
        }
      } catch (e) {
        manager.logger.error(`subscribeToQuery fullRefresh error for ${subscriptionKey}:`, e);
      } finally {
        isFullRunning = false;
      }
    };
    ...
    // ── runIncludesRefresh ───────────────────────────────────────────────
    let isIncludesRunning = false;
    const runIncludesRefresh = async () => {
      if (!ensureActive() || isIncludesRunning || !hasIncludes || currentRows.length === 0) return;
      isIncludesRunning = true;
      try {
        try { onLoading?.(); } catch { }
        const enriched = await resolveIncludes(currentRows, includes);
        if (ensureActive()) callback(enriched);
      } catch (e) {
        manager.logger.error(`subscribeToQuery includesRefresh error for ${subscriptionKey}:`, e);
      } finally {
        isIncludesRunning = false;
      }
    };

    // ── Debounces ─────────────────────────────────────────────────────────
    let mainDebounce: ReturnType<typeof setTimeout> | null = null;
    let includesDebounce: ReturnType<typeof setTimeout> | null = null;

    const debouncedFull = () => {
      if (mainDebounce !== null) clearTimeout(mainDebounce);
      mainDebounce = setTimeout(() => { mainDebounce = null; runFullRefresh(); }, SUBSCRIBE_DEBOUNCE_DELAY);
    };

    const debouncedIncludes = () => {
      if (includesDebounce !== null) clearTimeout(includesDebounce);
      includesDebounce = setTimeout(() => { includesDebounce = null; runIncludesRefresh(); }, SUBSCRIBE_DEBOUNCE_DELAY);
    };

    // ── Carga inicial ─────────────────────────────────────────────────────
    runFullRefresh();

    // ── Suscripción tabla principal ───────────────────────────────────────
    const mainSub = liveQuery(async () => await mainTable.count()).subscribe({
      next: debouncedFull,
      error: (e) => manager.logger.error(`main liveQuery error for ${subscriptionKey}:`, e),
    });

    // ── Suscripciones tablas de includes ──────────────────────────────────
    const includeSubs = [...includeTables].map(incTableId => {
      const t = (manager.db as any).table(incTableId);
      return liveQuery(async () => await t.count()).subscribe({
        next: debouncedIncludes,
        error: (e) => manager.logger.error(`include liveQuery error for ${incTableId}:`, e),
      });
    });

    // ── Registro en querySubscriptions ───────────────────────────────────
    const subEntry = {
      filters,
      options,
      callback: (rows: TableRecord[]) => { callback(rows); },
      unsubscribe: () => { },
      currentIds,
    };

    const tableSubs = manager.querySubscriptions.get(tableId) || [];
    tableSubs.push(subEntry as any);
    manager.querySubscriptions.set(tableId, tableSubs);

    // ── Cleanup ───────────────────────────────────────────────────────────
    return () => {
      canceled = true;

      if (mainDebounce !== null) { clearTimeout(mainDebounce); mainDebounce = null; }
      if (includesDebounce !== null) { clearTimeout(includesDebounce); includesDebounce = null; }

      try { mainSub.unsubscribe(); } catch { }
      includeSubs.forEach(s => { try { s.unsubscribe(); } catch { } });

      const current = manager.querySubscriptions.get(tableId) || [];
      const filtered = current.filter(s => s !== subEntry);
      if (filtered.length === 0) manager.querySubscriptions.delete(tableId);
      else manager.querySubscriptions.set(tableId, filtered);

      manager.logger.debug(`Unsubscribed from query: ${subscriptionKey}`);
    };
  }

  private collectRelatedTables(
    includes?: IncludeRelProps,
    tableId: string = '',
    collected: Set<string> = new Set()
  ): Set<string> {
    if (!includes) return collected;

    Object.entries(includes).forEach(([key, relProps]) => {
      const tableSource = relProps.tableSource || tableId;
      const field = this.getField(relProps.foreignKey || key, tableSource)!;
      const relationType = tableSource === tableId ? 'direct' : 'reverse';
      const relTableId = relationType === 'direct' ? field.relationTableIdentifier! : tableSource;
      collected.add(relTableId);

      // Recursivamente recolectar includes anidados
      if (relProps.includes) {
        this.collectRelatedTables(relProps.includes, relTableId, collected);
      }
    });
  ...
  subscribeToCount(
    ...
    this.count(tableId, filters).then(initialCount => {
      if (!canceled) callback(initialCount);
    });

    const table = (this.db as any).table(tableId);
    const sub = liveQuery(async () => await table.count()).subscribe({
      next: () => {
        if (canceled) return;
        ...
        debounceTimer = setTimeout(async () => {
          ...
          try {
            ...
            if (!canceled) callback(count);
          } catch (e) {
            this.logger.error(`subscribeToCount error for ${tableId}:`, e);
          }
        }, SUBSCRIBE_DEBOUNCE_DELAY);
      },
      error: (e) => this.logger.error(`subscribeToCount liveQuery error for ${subscriptionKey}:`, e),
    });
 ...
 */
  ...
  subscribeToTotal(
    ...
    this.total(tableId, fieldId, filters).then(initialTotal => {
      if (!canceled) { lastTotal = initialTotal; callback(initialTotal); }
    });
      ...
      next: () => {
        ...
        debounceTimer = setTimeout(async () => {
          ...
          try {
            ...
            if (!canceled && t !== lastTotal) { lastTotal = t; callback(t); }
          } catch (e) {
            this.logger.error(`subscribeToTotal error for ${tableId}.${fieldId}:`, e);
          }
        }, SUBSCRIBE_DEBOUNCE_DELAY);
      },
      error: (e) => this.logger.error(`subscribeToTotal liveQuery error for ${subscriptionKey}:`, e),
    });
 ...
 */
  ...
  resolveTableIdentifier = (raw: string | undefined, record: TableRecord): string => {
    if (!raw) return '';
  ...
  resolveQuery = (query: any, record: any): any => {
    if (!query) return {};

    return Object.fromEntries(
      Object.entries(query).map(([field, condition]: [string, any]) => {
        if (condition && typeof condition === 'object' && 'v' in condition) {
          const raw = condition.v;
          if (typeof raw !== 'string' || !raw.includes('{')) {
            return [field, condition];
          }

          const resolved = raw.replace(/\{([\w.]+)\}/g, (_, path) => {
            const val = path.split('.').reduce((obj: any, k: string) => obj?.[k], record);
            return val ?? undefined;
          });

          if (resolved === undefined || resolved.includes('undefined')) {
            return [field, { ...condition, v: '__unresolved__' }];
          }

          return [field, { ...condition, v: resolved }];
        }
        return [field, condition];
      })
    );
  ...
  // Devuelve un Table Field listo para usar en ColumnSelector o etc, para mostrar la info de si mismo en tablas.
  selfColumn(tableId: string): TableField {
    const table = this.getTable(tableId)!;
    return {
      id: '__meta_'+table.identifier,
      name: table.name,
      identifier: 'id',
      tableIdentifier: table.identifier,
      description: 'El propio elemento',
      fieldFormat: 'relation',
      relationTableIdentifier: table.identifier,
      isVisible: true,
    }
  }
  ...
  async waitFor<T>(
    ...
    expectedValue: T | T[],
    options?: {
      timeout?: number;        // tiempo máximo de espera en ms (default: 5000)
      interval?: number;       // intervalo entre comprobaciones (default: 50)
      checkEquality?: (actual: T, expected: T) => boolean; // función personalizada de comparación
    }
  ): Promise<T> {
    const {
      timeout = 5000,
      interval = 50,
      checkEquality = (actual: T, expected: T) => actual === expected
    } = options || {};
    ...
    return new Promise((resolve, reject) => {
      const check = () => {
        ...
        // Verificar si el valor actual coincide con alguno de los esperados
        const matches = expectedArray.some(expected =>
          checkEquality(currentValue, expected)
        );
        ...
        if (Date.now() - startTime > timeout) {
          reject(new Error(
            `Timeout esperando accessor después de ${timeout}ms. ` +
            `Valor actual: ${JSON.stringify(currentValue)}, ` +
            `Esperado: ${JSON.stringify(expectedArray)}`
          ));
          return;
        }

        // Volver a comprobar después del intervalo
